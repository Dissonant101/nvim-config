
" If you are distributing this theme, please replace this comment
" with the appropriate license attributing the original VS Code
" theme author.


" Lunar Pink - A nice dark theme

" ==========> Reset
set background=dark

hi clear

if exists("syntax_on")
  syntax reset
endif

let g:colors_name = 'lunar-pink'

" ==========> Highlight function
function! s:h(face, guibg, guifg, ctermbg, ctermfg, gui)
  let l:cmd="highlight " . a:face
  
  if a:guibg != ""
    let l:cmd = l:cmd . " guibg=" . a:guibg
  endif

  if a:guifg != ""
    let l:cmd = l:cmd . " guifg=" . a:guifg
  endif

  if a:ctermbg != ""
    let l:cmd = l:cmd . " ctermbg=" . a:ctermbg
  endif

  if a:ctermfg != ""
    let l:cmd = l:cmd . " ctermfg=" . a:ctermfg
  endif

  if a:gui != ""
    let l:cmd = l:cmd . " gui=" . a:gui
  endif

  exec l:cmd
endfun


" ==========> Colors dictionary

" GUI colors dictionary (hex)
let s:hex = {}
" Terminal colors dictionary (256)
let s:bit = {}

let s:hex.color0="#131313"
let s:hex.color1="#dddddd"
let s:hex.color2="#e621af"
let s:hex.color3="#404040"
let s:hex.color4="#1d1d1d"
let s:hex.color5="#454545"
let s:hex.color6="#272727"
let s:hex.color7="#595959"
let s:hex.color8="#fa35c3"
let s:hex.color9="#ff4edc"
let s:hex.color10="#5e5e5e"
let s:hex.color11="#009294"
let s:hex.color12="#313131"
let s:hex.color13="#fbfbfb"
let s:hex.color14="#e7e7e7"
let s:hex.color15="#363636"
let s:hex.color16="#09e4e6"
let s:hex.color17="#e09ece"
let s:hex.color18="#05a6a8"
let s:hex.color19="#df6cbe"

let s:bit.color10="30"
let s:bit.color17="37"
let s:bit.color15="44"
let s:bit.color9="59"
let s:bit.color2="163"
let s:bit.color18="169"
let s:bit.color16="182"
let s:bit.color7="205"
let s:bit.color8="206"
let s:bit.color12="231"
let s:bit.color0="233"
let s:bit.color4="234"
let s:bit.color5="235"
let s:bit.color11="236"
let s:bit.color14="237"
let s:bit.color3="238"
let s:bit.color6="240"
let s:bit.color1="253"
let s:bit.color13="254"


" ==========> General highlights 
call s:h("Normal", s:hex.color0, s:hex.color1, s:bit.color0, s:bit.color1, "none")
call s:h("Cursor", s:hex.color2, s:hex.color0, s:bit.color2, s:bit.color0, "none")
call s:h("Visual", s:hex.color3, "", s:bit.color3, "", "none")
call s:h("ColorColumn", s:hex.color4, "", s:bit.color4, "", "none")
call s:h("LineNr", "", s:hex.color5, "", s:bit.color3, "none")
call s:h("CursorLine", s:hex.color6, "", s:bit.color5, "", "none")
call s:h("CursorLineNr", "", s:hex.color7, "", s:bit.color6, "none")
call s:h("CursorColumn", s:hex.color6, "", s:bit.color5, "", "none")
call s:h("StatusLineNC", s:hex.color6, s:hex.color8, s:bit.color5, s:bit.color7, "none")
call s:h("StatusLine", s:hex.color3, s:hex.color9, s:bit.color3, s:bit.color8, "none")
call s:h("VertSplit", "", s:hex.color10, "", s:bit.color9, "none")
call s:h("Folded", s:hex.color6, s:hex.color11, s:bit.color5, s:bit.color10, "none")
call s:h("Pmenu", s:hex.color12, s:hex.color13, s:bit.color11, s:bit.color12, "none")
call s:h("PmenuSel", s:hex.color4, s:hex.color14, s:bit.color4, s:bit.color13, "none")
call s:h("EndOfBuffer", s:hex.color0, s:hex.color15, s:bit.color0, s:bit.color14, "none")
call s:h("NonText", s:hex.color0, s:hex.color15, s:bit.color0, s:bit.color14, "none")


" ==========> Syntax highlights
call s:h("Constant", "", s:hex.color16, "", s:bit.color15, "none")
call s:h("Special", "", s:hex.color16, "", s:bit.color15, "none")
call s:h("Identifier", "", s:hex.color17, "", s:bit.color16, "none")
call s:h("Function", "", s:hex.color18, "", s:bit.color17, "none")
call s:h("Statement", "", s:hex.color2, "", s:bit.color2, "none")
call s:h("Operator", "", s:hex.color1, "", s:bit.color1, "none")
call s:h("PreProc", "", s:hex.color2, "", s:bit.color2, "none")
call s:h("Type", "", s:hex.color2, "", s:bit.color2, "none")
call s:h("String", "", s:hex.color19, "", s:bit.color18, "none")
call s:h("Number", "", s:hex.color16, "", s:bit.color15, "none")

highlight link cStatement Statement
highlight link cSpecial Special


" Generated using https://github.com/nice/themeforge
" Feel free to remove the above URL and this line.
